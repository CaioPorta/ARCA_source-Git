...
class IHMmain(QWidget):
    #%% Init
    def __init__(self):
        self.APPVersion = "Versão Beta 0.0.5.5.33.2" #AnáliseDoMercado.Desempenho.ARCA.Tributação.NegociaçõesEGestãoDeDB.HMI
        self.OnlineVersion = ""
        super().__init__()
...
# HMI fixed parameters
        ...
        pixmap = QPixmap("./images/ThinkingMonkey.png")
        self.CustomCursor_ThinkingMonkey = QCursor(pixmap, 32, 32)
...
# Layout creation
        ...
        self.VersionUpdate = False

        self.CreatePage('0') # Página inicial do programa
        self.VersionChecker = VersionChecker.VersionChecker(self)

    def InicializacaoUsuario(self):
    ...
    def CreatePage_Decisao(self):
        if self.PageID == '26':
            if len(self.TextBox_NomeNovoAtivo.text())>0 and len(self.TextBox_NomeNovoAtivo.text())<7 and not self.TextBox_NomeNovoAtivo.text().upper() == "USD":
                self.setCursor(Qt.WaitCursor)
                if self.TextBox_NomeNovoAtivo.text() in self.DBManager.GetAllTickers(): # Otimizador
                    IsShare, self.Cache_Cotacao = self.YahooFinance.isShare(self.TextBox_NomeNovoAtivo.text())
                    self.unsetCursor()
                    if IsShare: self.CreatePage('34a')
                    else: self.CreatePage('35a')
                else:
                    # Perguntar se mesmo assim quer buscar
                    MessageBox_Msg1 = QMessageBox()
                    MessageBox_Msg1.setWindowTitle("Escolha um")
                    MessageBox_Msg1.setText("O ativo não está contido na lista de pré-checagem.\nDeseja mesmo assim tentar buscar a cotação online?\n(Isso pode levar alguns minutos)")
                    MessageBox_Msg1.setIcon(QMessageBox.Warning)
                    MessageBox_Msg1.setStandardButtons(QMessageBox.No|QMessageBox.Yes|QMessageBox.Cancel)
                    ...
                    if returnValue == QMessageBox.Yes:
                        ...
                        IsShare = False; self.Cache_Cotacao = 0.
                        self.unsetCursor()
                        if IsShare: self.CreatePage('34a')
                        else: self.CreatePage('35a')
            else:
            ...
            if len(self.TextBox_NovoNomeAtivo.text())>0 and len(self.TextBox_NovoNomeAtivo.text())<7 and not self.TextBox_NovoNomeAtivo.text().upper() == "USD":
                self.setCursor(Qt.WaitCursor)
                if self.TextBox_NovoNomeAtivo.text() in self.DBManager.GetAllTickers(): # Otimizador
                    IsShare, self.Cache_Cotacao = self.YahooFinance.isShare(self.TextBox_NovoNomeAtivo.text())
                    self.unsetCursor()
                    if IsShare: self.CreatePage('34b')
                    else: self.CreatePage('35b')
                else:
                    # Perguntar se mesmo assim quer buscar
                    MessageBox_Msg1 = QMessageBox()
                    MessageBox_Msg1.setWindowTitle("Escolha um")
        ...
        if self.OutraJanelaAberta:
            ...
            if PageID == '0':
                if self.PageIDAux == '17b':
                    if self.TextBox_CodigoDeConfirmacao.text() == self.CodigoDeConfirmacaoGerado:
                        if not self.DBManager.FLAG and not self.AtualizarCotacoes_running:
                            ...
                            if not self.LoggedIn:
                                ...
                                os.execl(sys.executable, sys.executable, *sys.argv) # Reboot app | It doesn't work when running by script, only by .exe
                    # self.InicializacaoUsuario()
                        else:
                    ...
                    self.PageID = '0'
                    self.PageIDAux = ''
                    self.InsertGridLayout(0, 0, 1, 20)
                    self.InicializacaoUsuario()
                    self.CreatePage0()
                    self.PutHeader()
                else:
                ...
                if self.PageID == '7':
                    if (self.TextBox_NovaSenha1.text() == self.TextBox_NovaSenha2.text() and
                        len(self.TextBox_NovaSenha1.text())>3): # Condições para passar pra página 1
                            ...
                            self.CreatePage1()
                            self.PutHeader()
                            self.YahooFinance.Thread_GetAllTickers()
                    ...
                    if self.TextBox_CodigoDeConfirmacao.text() == self.CodigoDeConfirmacaoGerado: # Condições para passar pra página 1
                        ...
                        if self.LoggedIn:
                            ...
                            self.Email = self.TextBox_Email.text().lower()
            ...
            #%% PageID == '6'
                ...
                if isEmail: # Recovering By Email
                    if self.DBManager.CheckIfEmailExists(self.TextBox_EmailOuUsuario.text().lower()):
                        self.Email = self.TextBox_EmailOuUsuario.text().lower()
                ...
                if (len(self.TextBox_NomeDeUsuario.text())>0 and
                    ...
                    not self.TextBox_Senha1.text().isalpha()): # Condições para passar pra página 8
                        ...
                        if not self.TextBox_Senha1.text() == self.TextBox_Senha2.text():
                            ...
                            self.F_GLayout[self.GCount-1].addWidget(self.Label_Erro3, 10, 1, 1, 1, Qt.AlignLeft)
                            self.TextBox_NomeDeUsuario.setFocus()
            elif PageID == 'ImportFromExcel':
                if self.TextBox_CodigoDeConfirmacao.text() == self.CodigoDeConfirmacaoGerado: # Condições para passar pra página 1
                    # self.ClearPage()
                    # self.PageID = 'ImportFromExcel'
                    # self.PageIDAux = ''
                    # self.InsertGridLayout(1, 0, 1, 20)
                    # self.CreatePageImportFromExcel()
                    # self.PutHeader()
            ...
            #%% PageID == '9'
                ...
                if PageID == '9_':
                    if self.PageIDAux == '15':
                        ...
                        if not self.DBManager.FLAG and not self.AtualizarCotacoes_running:
                            if self.TextBox_CodigoDeConfirmacao.text() == self.CodigoDeConfirmacaoGerado: # Condições para passar pra página 9
                                # Mudar Email
                                self.DBManager.UpdateEmail(self.TextBox_Email.text())
                                self.ClearPage()
                                self.PageIDAux = '9'
                                self.InsertGridLayout(1, 0, 1, 3)
                                self.CreatePage9()
            ...
            #%% PageID == '28'
                        ...
                        if data<=datetime.now():
                            ...
                            if not self.DBManager.FLAG and not self.AtualizarCotacoes_running:
                                try:
                                    ...
                                    self.ComboBox_Ativos.clear()
                                    ativos = self.DBManager.GetAtivos()
                                    for idx, ativo in enumerate(ativos):
                                        self.ComboBox_Ativos.addItem(ativo)
                                        if ativo == self.TextBox_NomeNovoAtivo.text():
                                            self.idxAtivo = idx
                                    self.ComboBox_Ativos.setCurrentIndex(self.idxAtivo)
                ...
                elif len(self.ComboBox_Ativos)>0:
                    self.ClearPage()
                    self.PageID = '28'
                    self.PageIDAux = ''
                    self.InsertGridLayout(1, 0, 1, 20)
    ...
    def LimparPage(self):
        if self.PageID in ['28', '29', '36a']:
            ...
            self.TextBox_Taxa.setText('0')
            self.TextBox_MoedaDaTaxa.setText(self.DBManager.GetUserCoinCurrency())
            self.TextBox_Conversao.setText('1')
            self.TextBox_Obs.setText('')
    ...
    def CreatePage0(self):
        ...
        self.Button_LogIn.setStyleSheet("QPushButton {background-color: rgb(20, 120, 30); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_LogIn.setToolTip("Log in")
        ...
        self.Button_EsqueciASenha.setToolTip("Esqueci a senha")
        self.Button_EsqueciASenha.setStyleSheet('QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_ImportarUsuario.setToolTip("Importar usuário")
        self.Button_ImportarUsuario.setStyleSheet('QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_CriarUsuario.setToolTip("Criar novo usuário")
        self.Button_CriarUsuario.setStyleSheet('QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_VersionInfo.setStyleSheet("QPushButton {background-color: none; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_VersionInfo.setToolTip("Versão")
        ...
        self.Button_MercadoInvisivel.setStyleSheet('QPushButton {background-color: rgba(0, 0, 0, 0); color: white; border: 1px solid rgba(0, 0, 0, 0); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_MercadoInvisivel.setToolTip("Ir para aba Mercado")
        ...
        self.Button_Mercado.setStyleSheet('QPushButton {background-color: black; color: white; border: none; outline: } QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_Mercado.setToolTip("Ir para aba Mercado")
    ...
    def CreatePage1(self):
        ...
        self.Button_Negociações.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_Negociações.setToolTip("Ir para aba Negociações")

        self.Button_NegociaçõesInvisivel = QPushButton()
        self.Button_NegociaçõesInvisivel.pressed.connect(lambda: self.CreatePage('10'))
        self.Button_NegociaçõesInvisivel.setStyleSheet('QPushButton {background-color: rgba(0, 0, 0, 0); color: white; border: 1px solid rgba(0, 0, 0, 0); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_NegociaçõesInvisivel.setToolTip("Ir para aba Negociações")
        ...
        self.Button_Tributacao.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_Tributacao.setToolTip("Ir para aba Tributação")

        self.Button_TributacaoInvisivel = QPushButton()
        self.Button_TributacaoInvisivel.pressed.connect(lambda: self.CreatePage('51'))
        self.Button_TributacaoInvisivel.setStyleSheet('QPushButton {background-color: rgba(0, 0, 0, 0); color: white; border: 1px solid rgba(0, 0, 0, 0); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_TributacaoInvisivel.setToolTip("Ir para aba Tributação")
        ...
        self.Button_ARCA.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_ARCA.setToolTip("IR para aba ARCA")

        self.Button_ARCAInvisivel = QPushButton()
        self.Button_ARCAInvisivel.pressed.connect(lambda: self.CreatePage('11'))
        self.Button_ARCAInvisivel.setStyleSheet('QPushButton {background-color: rgba(0, 0, 0, 0); color: white; border: 1px solid rgba(0, 0, 0, 0); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_ARCAInvisivel.setToolTip("Ir para aba ARCA")
        ...
        self.Button_Desempenho.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_Desempenho.setToolTip("Ir para aba Desempenho")

        self.Button_DesempenhoInvisivel = QPushButton()
        self.Button_DesempenhoInvisivel.pressed.connect(lambda: self.CreatePage('12'))
        self.Button_DesempenhoInvisivel.setStyleSheet('QPushButton {background-color: rgba(0, 0, 0, 0); color: white; border: 1px solid rgba(0, 0, 0, 0); border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_DesempenhoInvisivel.setToolTip("Ir para aba Desempenho")
        ...
        self.Button_Mercado.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        self.Button_Mercado.setFixedSize(int(self.frameGeometry().width()/3*.96),int(self.frameGeometry().height()/5))
        self.Button_Mercado.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_Mercado.setToolTip("Ir para aba Análise do Mercado")
        ...
        self.Button_MercadoInvisivel.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_MercadoInvisivel.setToolTip("Ir para aba Análise do Mercado")
    ...
    def CreatePage3(self):
        ...
        self.Button_ImportDBToExcel = QPushButton()
        self.Button_ImportDBToExcel.pressed.connect(lambda: self.CreatePage('3b'))
        self.Button_ImportDBToExcel.setStyleSheet('QPushButton {background-color: rgb(0, 0, 0); color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        self.Button_ImportDBToExcel.setFixedSize(int(self.frameGeometry().width()*2/5),int(self.frameGeometry().height()-160))
        self.Button_ImportDBToExcel.setIconSize(QSize(int(self.frameGeometry().width()*2/5),int(self.frameGeometry().height())))
        self.Button_ImportDBToExcel.setIcon(QIcon("./images/import_from_excel_icon.png"))
        self.Button_ImportDBToExcel.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_ImportDBToExcel.setToolTip(r"Importar base de dados via arquivo .xlsx")

        self.Button_ImportDBToDB = QPushButton()
        self.Button_ImportDBToDB.pressed.connect(lambda: self.OnButtonPressed('ImportFromDB'))
        self.Button_ImportDBToDB.setStyleSheet('QPushButton {background-color: rgb(0, 0, 0); color: white; border: 1px solid rgb(0, 0, 0)} QToolTip {background-color: black; color: white;border: black solid 1px}')
        self.Button_ImportDBToDB.setFixedSize(int(self.frameGeometry().width()*2/5),int(self.frameGeometry().height()-160))
        self.Button_ImportDBToDB.setIconSize(QSize(int(self.frameGeometry().width()*2/5),int(self.frameGeometry().height())))
        self.Button_ImportDBToDB.setIcon(QIcon("./images/import_from_db_icon.png"))
        self.Button_ImportDBToDB.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_ImportDBToDB.setToolTip(r"Importar base de dados via arquivo .db")
        ...
        self.F_GLayout[self.GCount-1].addWidget(self.Button_ImportDBToExcel, 1, 0, 2, 1, Qt.AlignTop)
        self.F_GLayout[self.GCount-1].addWidget(self.Button_ImportDBToDB, 1, 1, 2, 1, Qt.AlignTop)

    def CreatePage3b(self):
        self.Background_1 = QLabel()
        Background_1 = 'rgb(0, 0, 0)'
        self.Background_1.setStyleSheet("background-color: "+Background_1)

        self.Label_Titulo = QLabel('Importação de usuário')
        self.Label_Titulo.setStyleSheet('color: white')
        self.Label_Titulo.setFont(self.font30)
        self.Label_Titulo.setAlignment(Qt.AlignCenter)
        ...
        self.TextBox_NomeDeUsuario = QLineEdit()
        self.TextBox_NomeDeUsuario.returnPressed.connect(lambda: self.CreatePage('8b'))
        self.TextBox_NomeDeUsuario.setFixedWidth(int(self.frameGeometry().width()/3*0.9))
        ...
        self.TextBox_Email.returnPressed.connect(lambda: self.CreatePage('8b'))
        ...
        self.TextBox_Senha1.returnPressed.connect(lambda: self.CreatePage('8b'))
        ...
        self.TextBox_Senha2.returnPressed.connect(lambda: self.CreatePage('8b'))
        ...
        self.Button_PathPadrao.setStyleSheet("QPushButton {background-color: green; color: white;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_PathPadrao.setFont(self.font16)
        self.Button_PathPadrao.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_PathPadrao.setToolTip(r"Usar o caminho padrão")
        ...
        self.Button_PathEspecifico.setStyleSheet("QPushButton {background-color: black; color: white;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_PathEspecifico.setFont(self.font16)
        self.Button_PathEspecifico.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_PathEspecifico.setToolTip(r"Informar o caminho específico")
        ...
        self.Button_ConfirmacaoDeEmail.pressed.connect(lambda: self.CreatePage('8b'))
        self.Button_ConfirmacaoDeEmail.setFont(self.font16)
        self.Button_ConfirmacaoDeEmail.setIconSize(QSize(int(self.frameGeometry().height()/10*0.9),int(self.frameGeometry().height()/10*0.9)))
        self.Button_ConfirmacaoDeEmail.setFixedWidth(int(self.frameGeometry().width()/3*0.9))
        self.Button_ConfirmacaoDeEmail.setIcon(QIcon("./images/log_in.png"))
        self.Button_ConfirmacaoDeEmail.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_ConfirmacaoDeEmail.setStyleSheet("QPushButton {background-color: rgb(20, 120, 30)} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_ConfirmacaoDeEmail.setToolTip(r"Próxima etapa")
    ...
    def CreatePage6(self, Email, NomeDeUsuario):
        ...
        self.Label_Msg1 = QLabel('Mandamos para '+Email[:3]+'***@***'+Email[-4:]+' um código de confirmação da conta.')
        self.Label_Msg1.setStyleSheet('color: white')
        self.Label_Msg1.setFont(self.font16)
        self.Label_Msg1.setAlignment(Qt.AlignCenter)
        ...
        self.Button_ResendEmail.setToolTip(r"Clique aqui se não recebeu o email")
        self.Button_ResendEmail.setStyleSheet('QToolTip {background-color: black; color: white;border: black solid 1px}')
    ...
    def CreatePage8b(self):
        ...
        self.TextBox_CodigoDeConfirmacao = QLineEdit()
        self.TextBox_CodigoDeConfirmacao.returnPressed.connect(lambda: self.CreatePage('ImportFromExcel'))
        self.TextBox_CodigoDeConfirmacao.setFixedWidth(int(self.frameGeometry().width()/3*0.9))
        ...
        self.Button_ConfirmacaoDeEmail = QPushButton('Confirmar')
        self.Button_ConfirmacaoDeEmail.pressed.connect(lambda: self.CreatePage('ImportFromExcel'))
        self.Button_ConfirmacaoDeEmail.setFont(self.font16)
        self.Button_ConfirmacaoDeEmail.setIconSize(QSize(int(self.frameGeometry().height()/10*0.9),int(self.frameGeometry().height()/10*0.9)))
        self.Button_ConfirmacaoDeEmail.setFixedWidth(int(self.frameGeometry().width()/3*0.9*2/3))
        self.Button_ConfirmacaoDeEmail.setIcon(QIcon("./images/log_in.png"))
    ...
    def CreatePageImportFromExcel(self):
        self.Background_1 = QLabel()
        Background_1 = 'rgb(0, 0, 0)'
        self.Background_1.setStyleSheet("background-color: "+Background_1)

        self.Label_Titulo = QLabel('Importando dados do excel')
        self.Label_Titulo.setStyleSheet('color: white')
        self.Label_Titulo.setFont(self.font30)
        self.Label_Titulo.setAlignment(Qt.AlignCenter)
    ...
    def CreatePage9(self):
        ...
        self.Button_ProfilePhoto.setToolTip(r"Clique aqui para escolher uma foto")
        self.Button_ProfilePhoto.setStyleSheet('QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_VersionInfo.setIcon(QIcon("./images/Info.png"))
        self.Button_VersionInfo.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_VersionInfo.setStyleSheet("QPushButton {background-color: black; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_VersionInfo.setToolTip(r"Versão")
        ...
        self.Button_ExportDBToExcel.setStyleSheet('QPushButton {background-color: rgb(0, 0, 0); color: white; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_ExportDBToExcel.setToolTip(r"Exportar base de dados no formato .xlsx")

        self.Button_ExportDBToDB = QPushButton()
        self.Button_ExportDBToDB.pressed.connect(lambda: self.OnButtonPressed('ExportToDB'))
        self.Button_ExportDBToDB.setStyleSheet('QPushButton {background-color: rgb(0, 0, 0); color: white; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}')
        ...
        self.Button_ExportDBToDB.setToolTip(r"Exportar base de dados no formato .db")
    ...
    def CreatePage15(self):
        ...
        self.Button_Alterar.setStyleSheet("QPushButton {background-color: rgb(20, 120, 30)} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_Alterar.setToolTip(r"Alterar")
    ...
    def CreatePage17a(self):
        Email = self.TextBox_Email.text().lower()
        NomeDeUsuario = self.DBManager.User
        # Envio de email com o código
        def Thread(threadname, parametros):
            self.setCursor(Qt.WaitCursor)
            Email, NomeDeUsuario = parametros
    ...
    def CreatePage45(self):
        ...
        self.Button_Update.setFont(self.font22)
        self.Button_Update.setIconSize(QSize(int(self.frameGeometry().height() / 10 * 0.9), int(self.frameGeometry().height() / 10 * 0.9)))
        self.Button_Update.setIcon(QIcon("./images/update.png"))
        self.Button_Update.setCursor(QCursor(Qt.PointingHandCursor))
        self.Button_Update.setStyleSheet("background-color: rgb(120, 120, 120)")
        self.Button_Update.pressed.connect(lambda: self.OnButtonPressed('RemoveUnusedAtivosFromCotacoes'))
        ...
        self.F_GLayout[self.GCount-1].addWidget(self.Button_Update, 1, 3, 1, 1, Qt.AlignCenter)
        ...
        if len(Ativos)>0:
            ...
            self.Button_Joinha.setStyleSheet('QPushButton {background-color: black; color: white; border: 1px solid black} QToolTip {background-color: black; color: white;border: black solid 1px}')
            self.Button_Joinha.setIcon(QIcon("./images/Joinha.png"))
            self.Button_Joinha.setToolTip(r"Ok")
    ...
    def CreatePageInfo27(self): MessageBox_Msg1 = QMessageBox.about(self,'Informação',"                                             Ativos Internacionais\n\nNada mais rentável do que negócios em dólar.\nProteção à crises locais.\n\nO que olhar:\nLucratividade, endividamento, preço, crescimento, governança corporativa.\n\nChecklist:\nP/L; PEG RATIO; P/VP; ROE; ROA; ROIC; MG. BRUTA; MG. LUCRO;\nDL/PL; DL/EBIT; LIQUIDEZ CORRENTE; DIVIDA BRUTA/EBIT;\nCAGR EBIT; CAGR LUCRO; SALDO DOS ACIONISTAS.\n\n                                                 Criptomoedas\n\nExposição de altíssimo risco.\nMuito volátil, aplicação convexa.\n\nDica de estratégia saudável:\n50 a 60% em BTC\n10% em ETH\n10% em BNB\nO restante divide-se nas pimentinhas.")

    #%% UpdatePage
    def UpdatePage(self):
        if self.PageID == '18':
            UserCoin = self.DBManager.GetCorretoraCoinCurrency()
            corretora = self.ComboBox_Corretoras.currentText()
            self.idxCorretora = self.ComboBox_Corretoras.currentIndex()
            self.Label_Title2.setText(corretora)
            self.TextBox_Msg2.setText(UserCoin+' ~'+str(self.DBManager.GetPatrimonioTotalCorretora()))
            Investido = round(self.DBManager.GetTotalInvestidoEmCorretora(corretora, "Bolsa"), 2)
            self.TextBox_Msg6.setText(UserCoin+' ~'+str(Investido))
            self.TextBox_Msg3.setText(UserCoin+' '+str(self.DBManager.GetValorEmContaCorrente()))
            self.TextBox_Msg4.setText(self.DBManager.GetDataUltimaOPRegistrada())
            valor = round(self.DBManager.GetUltimoDepositoOuSaqueRegistrado(corretora),2)
            self.TextBox_Msg5.setText(UserCoin+' '+str(valor))
            if valor >= 0: self.TextBox_Msg5.setStyleSheet('QLineEdit {background-color: black; color: green;}')
            else: self.TextBox_Msg5.setStyleSheet('QLineEdit {background-color: black; color: orange;}')
        elif self.PageID == '19':
            UserCoin = self.DBManager.GetCorretoraCoinCurrency()
            corretora = self.ComboBox_Corretoras.currentText()
            self.Label_Title2.setText(corretora)
            self.TextBox_Msg2.setText(UserCoin+' ~'+str(self.DBManager.GetPatrimonioTotalCorretora()))
            Investido = round(self.DBManager.GetTotalInvestidoEmCorretora(corretora, "Cripto"), 2)
            self.TextBox_Msg6.setText(UserCoin+' ~'+str(Investido))
            self.TextBox_Msg3.setText(UserCoin+' '+str(self.DBManager.GetValorEmContaCorrente()))
            self.TextBox_Msg4.setText(self.DBManager.GetDataUltimaOPRegistrada())
            valor = self.DBManager.GetUltimoDepositoOuSaqueRegistrado()
            self.TextBox_Msg5.setText(UserCoin+' '+str(valor))
            if valor >= 0: self.TextBox_Msg5.setStyleSheet('QLineEdit {background-color: black; color: green;}')
            else: self.TextBox_Msg5.setStyleSheet('QLineEdit {background-color: black; color: orange;}')
        elif self.PageID == 'Bancos':
            self.idxBanco = self.ComboBox_Bancos.currentIndex()
            self.Label_Title2.setText(self.ComboBox_Bancos.currentText())
            self.TextBox_Msg2.setText(self.DBManager.GetBancoCoinCurrency(self.ComboBox_Bancos.currentText())+' '+str(self.DBManager.GetValorEmContaCorrente_Banco(self.ComboBox_Bancos.currentText())))
            self.Label_CC.setText("Valor atualizado ("+self.DBManager.GetBancoCoinCurrency(self.ComboBox_Bancos.currentText())+")")
        elif self.PageID == '23':
            self.Label_Msg1.setText('Operações realizadas com '+self.ComboBox_Ativos.currentText())
            ...
            self.idxAtivo = self.ComboBox_Ativos.currentIndex()
            ...
            UserCoin = self.DBManager.GetUserCoinCurrency()
            self.GraphWidget_MainGraph.deleteLater()
            if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page46_MainGraph_VisaoGeral()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page46_MainGraph_AtivoPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por setor":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page46_MainGraph_AtivoPorSetor()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            self.HMI_ARCA.Custo = 0
            self.HMI_ARCA.Retorno = 0
            if self.ComboBox_Ativos.currentText() == "Todos":
                for ativo in self.HMI_ARCA.ativos:
                    self.HMI_ARCA.Custo += self.DBManager.GetCustoAtivo(ativo, "Bolsa")
                    self.HMI_ARCA.Retorno += self.DBManager.GetEstoqueAtivo(ativo, "Bolsa") * self.DBManager.GetCotacao(ativo)
                coins = self.DBManager.GetAllAtivos_Cripto()
            elif self.ComboBox_Ativos.currentText() == "Criptos":
                coins = self.DBManager.GetAllAtivos_Cripto()
            else:
                self.HMI_ARCA.Custo = self.DBManager.GetCustoAtivo(self.ComboBox_Ativos.currentText(), "Bolsa")
                self.HMI_ARCA.Retorno = self.DBManager.GetEstoqueAtivo(self.ComboBox_Ativos.currentText(), "Bolsa") * self.DBManager.GetCotacao(self.ComboBox_Ativos.currentText())
            if self.ShowValues:
                self.TextBox_Custo.setText('Custo: ' + UserCoin + ' ' + str(f'{round(self.HMI_ARCA.Custo, 2):.2f}'))
                self.TextBox_Retorno.setText('Retorno: ' + UserCoin + ' ' + str(f'{round(self.HMI_ARCA.Retorno, 2):.2f}'))
            else:
                self.TextBox_Custo.setText('Custo: X')
                self.TextBox_Retorno.setText('Retorno: '+str(round((self.HMI_ARCA.Retorno-self.HMI_ARCA.Custo)/self.HMI_ARCA.Custo*100,1))+'%X')
            if self.HMI_ARCA.Retorno >= self.HMI_ARCA.Custo:
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: green;}')
            else:
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: red;}')
            self.GraphWidget_Subtipos.deleteLater()
            self.GraphWidget_Subtipos = self.HMI_ARCA.CreateGraph_Page46_Subtipos()
            self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_Subtipos, 7, 0, 3, 3, Qt.AlignCenter | Qt.AlignBottom)

        elif self.PageID == '47':
            UserCoin = self.DBManager.GetUserCoinCurrency()
            self.GraphWidget_MainGraph.deleteLater()
            if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page47_MainGraph_VisaoGeral()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page47_MainGraph_AtivoPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por setor":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page47_MainGraph_AtivoPorSetor()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)
            ...
            if self.HMI_ARCA.Retorno >= self.HMI_ARCA.Custo:
                ...
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: red;}')
            self.GraphWidget_Subtipos.deleteLater()
            self.GraphWidget_Subtipos = self.HMI_ARCA.CreateGraph_Page47_Subtipos()
            self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_Subtipos, 7, 0, 3, 3, Qt.AlignCenter | Qt.AlignBottom)
            ...
            self.GraphWidget_MainGraph.deleteLater()
            if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page48_MainGraph_VisaoGeral()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page48_MainGraph_AtivoPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por setor":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page48_MainGraph_AtivoPorSetor()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)
            ...
            if self.HMI_ARCA.Retorno >= self.HMI_ARCA.Custo:
                ...
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: red;}')
            self.GraphWidget_Subtipos.deleteLater()
            self.GraphWidget_Subtipos = self.HMI_ARCA.CreateGraph_Page48_Subtipos()
            self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_Subtipos, 7, 0, 3, 3, Qt.AlignCenter | Qt.AlignBottom)
            ...
            self.GraphWidget_MainGraph.deleteLater()
            if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page49_MainGraph_VisaoGeral()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page49_MainGraph_AtivoPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por setor":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page49_MainGraph_AtivoPorSetor()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)
            ...
            if self.ComboBox_Ativos.currentText() == "Todos":
                for ativo in self.HMI_ARCA.ativos:
                    ...
                    self.HMI_ARCA.Retorno += self.DBManager.GetEstoqueAtivo(ativo, "Bolsa") * self.DBManager.GetCotacao(ativo)
                coins = self.DBManager.GetAllAtivos_Cripto()
                for ativo in coins: self.HMI_ARCA.Custo += self.DBManager.GetCustoAtivo(ativo, "Cripto")
                for ativo in coins: self.HMI_ARCA.Retorno += self.DBManager.GetEstoqueAtivo(ativo, "Cripto") * self.DBManager.GetCotacao(ativo)
            elif self.ComboBox_Ativos.currentText() == "Criptos":
                coins = self.DBManager.GetAllAtivos_Cripto()
                for ativo in coins: self.HMI_ARCA.Custo += self.DBManager.GetCustoAtivo(ativo, "Cripto")
                for ativo in coins: self.HMI_ARCA.Retorno += self.DBManager.GetEstoqueAtivo(ativo, "Cripto") * self.DBManager.GetCotacao(ativo)
            ...
            if self.HMI_ARCA.Retorno >= self.HMI_ARCA.Custo:
                ...
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: red;}')
            self.GraphWidget_Subtipos.deleteLater()
            self.GraphWidget_Subtipos = self.HMI_ARCA.CreateGraph_Page49_Subtipos()
            self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_Subtipos, 7, 0, 3, 3, Qt.AlignCenter | Qt.AlignBottom)
            ...
            self.GraphWidget_MainGraph.deleteLater()
            if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page50_MainGraph_VisaoGeral()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Ativo por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page50_MainGraph_AtivoPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)

            elif self.HMI_ARCA.MainGraphIsShowing == "Total de UserCoin por corretora":
                self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page50_MainGraph_TotalDeUserCoinPorCorretora()
                self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)
            ...
            if self.ComboBox_Ativos.currentText() == "Todos":
                for ativo in self.HMI_ARCA.ativos:
                    self.HMI_ARCA.Custo += self.DBManager.GetCustoAtivo(ativo, "Cripto")
                    self.HMI_ARCA.Retorno += self.DBManager.GetEstoqueAtivo(ativo, "Cripto") * self.DBManager.GetCotacao(ativo)
            else:
                self.HMI_ARCA.Custo = self.DBManager.GetCustoAtivo(self.ComboBox_Ativos.currentText(), "Cripto")
                self.HMI_ARCA.Retorno = self.DBManager.GetEstoqueAtivo(self.ComboBox_Ativos.currentText(), "Cripto") * self.DBManager.GetCotacao(self.ComboBox_Ativos.currentText())
            if self.ShowValues:
                self.TextBox_Custo.setText('Custo: ' + UserCoin + ' ' + str(f'{round(self.HMI_ARCA.Custo, 2):.2f}'))
                self.TextBox_Retorno.setText('Retorno: ' + UserCoin + ' ' + str(f'{round(self.HMI_ARCA.Retorno, 2):.2f}'))
            else:
            ...
            if self.HMI_ARCA.Retorno >= self.HMI_ARCA.Custo:
                ...
                self.TextBox_Retorno.setStyleSheet('QLineEdit {background-color: black; color: red;}')
            self.GraphWidget_Distribuicao.deleteLater()
            self.GraphWidget_Distribuicao = self.HMI_ARCA.CreateGraph_Page50_Distribuicao()
            self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_Distribuicao, 7, 0, 3, 3, Qt.AlignCenter | Qt.AlignBottom)
    ...
    def GetSizeOfTableColumns(self, data, HHeader):
        ...
        if self.PageIDAux == '45':
            col = len(max(coluna[0], key=len))*16 # Ativo
            resposta.append(col)
            col = len(max(coluna[1], key=len))*16 # Data
            resposta.append(col)
            col = len(max(coluna[2], key=len))*16 # Cotação em CURRENCY
            resposta.append(col)

        elif self.PageID == '11':
            col = len(max(coluna[0], key=len))*10 # Corretora
            resposta.append(col)
            col = len(max(coluna[1], key=len))*9 # Conta-corrente
            resposta.append(col)
            ...
            if not col8 >= len(max(coluna[8], key=len))*10:
                ...
                col8 = len(max(coluna[8], key=len))*8.5
            else:
                self.Table_Operacoes_RealizadasHeader.setColumnWidth(8, col8)

            resposta.append(col8)

        elif self.PageID in ['43', '44']:
            col8 = int(self.frameGeometry().width())
            col = len(max(coluna[0], key=len))*8
            col8 -= col
            resposta.append(col)
            ...
            if not col8 >= len(max(coluna[8], key=len))*10:
                self.Table_Operacoes_RealizadasHeader.setColumnWidth(8, col8)
                col8 = len(max(coluna[8], key=len))*8
            else:
                self.Table_Operacoes_RealizadasHeader.setColumnWidth(8, col8)
    ...
    def ClearPage(self):
        if self.VersionUpdate:
            self.VersionUpdate = False
            MessageBox_Msg1 = QMessageBox()
            MessageBox_Msg1.setWindowTitle("Atualização disponível")
            MessageBox_Msg1.setText("Esta versão é a "+self.APPVersion+".\nExiste uma versão mais recente disponível ("+self.OnlineVersion+").\nDeseja atualizar agora?\nTambém é possível acessar\nhttps://github.com/CaioPorta/ARCA/tree/main/dist\npara atualizar manualmente.")
            MessageBox_Msg1.setIcon(QMessageBox.Warning)
            MessageBox_Msg1.setStandardButtons(QMessageBox.No | QMessageBox.Yes)
            MessageBox_Msg1.setDefaultButton(QMessageBox.Yes)
            MessageBox_Msg1.setWindowIcon(QIcon(".\images\logoARCA.png"))
            returnValue = MessageBox_Msg1.exec()
            if returnValue == QMessageBox.Yes:
                self.LW = LoadingWindow.LoadingWindow(self)
                self.LW.show()
                self.VersionChecker.StartThread_UpdateVersion()
                while self.VersionChecker.InProgress:
                    self.LW.Label_Titulo.setText(self.VersionChecker.Title)
                    self.LW.pbar.setValue(self.VersionChecker.Progress)
                    time.sleep(0.2)
                self.LW.close()
            MessageBox_Msg1 = QMessageBox()
            MessageBox_Msg1.setWindowTitle("Reinicialização")
            MessageBox_Msg1.setText("Reinicialização da ARCA necessária.\nUm atalho foi criado na área de tabalho.\nClique em OK para reiniciar.")
            MessageBox_Msg1.setIcon(QMessageBox.Warning)
            MessageBox_Msg1.setStandardButtons(QMessageBox.OK)
            MessageBox_Msg1.setDefaultButton(QMessageBox.OK)
            MessageBox_Msg1.setWindowIcon(QIcon(".\images\logoARCA.png"))
            returnValue = MessageBox_Msg1.exec()
            # Close APP
            self.DBManager.Disconnect()
            currdir = os.getcwd()
            os.startfile(currdir + "\\ARCA.exe")
            sys.exit()
        ...
        try: self.GLayout.removeItem(self.HBoxLayout_Header)
        except:pass
        ...
        if self.PageID == "11" and self.PageIDAux == '':
            ...
            try:
                ...
                self.F_GLayout[0].removeItem(self.VBoxLayout_Ativo)
            except:pass
        ...
        elif self.PageID in ['28', '29'] and self.PageIDAux == '':
            try:
                self.Button_Calendario.deleteLater()
                self.Button_Relogio.deleteLater()
                ...
                self.Button_Desdobramento.deleteLater()
                self.Button_Grupamento.deleteLater()
                self.Button_Bonificacao.deleteLater()
        ...
        elif self.PageID in ['36a', '36b'] and self.PageIDAux == '':
            try:
                self.Button_Calendario.deleteLater()
                self.Button_Relogio.deleteLater()
                self.Button_Info3.deleteLater()
            ...
            try:
                ...
                self.Button_BuscarConversao.deleteLater()
    ...
    def PutHeader(self):
        ...
        self.Button_Return.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_Return.setIcon(QIcon("./images/return_icon.png"))
        self.Button_Return.setToolTip(r"Voltar")
        ...
        self.Button_Home.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_Home.setIcon(QIcon("./images/home_icon.png"))
        self.Button_Home.setToolTip(r"Início")

        self.Button_ShowHideValues = QPushButton()
        self.Button_ShowHideValues.setToolTip(r"Mostrar ou ocultar valores")
        ...
        self.Button_ShowHideValues.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        ...
        self.Button_Sair.setToolTip(r"Sair")
        ...
        self.Button_Sair.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_Sair.setIcon(QIcon("./images/exit_icon.png"))

        self.Button_AttCotacoesManuais = QPushButton()
        self.Button_AttCotacoesManuais.setToolTip(r"Atualizar cotações manuais (C)")
        ...
        self.Button_AttCotacoesManuais.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        ...
        self.Button_AttCotacoes.setStyleSheet("QToolTip {background-color: black; color: white;border: black solid 1px}")
        self.Button_AttCotacoes.setToolTip(r"Atualizar cotações")
        ...
        self.Button_Perfil.setToolTip(r"Perfil e ferramentas")
        ...
        self.Button_Perfil.setStyleSheet("QPushButton {background-color: "+Background_Header+"; border: none; outline: none;} QToolTip {background-color: black; color: white;border: black solid 1px}")
        try:
            if os.path.exists(os.getcwd()+"\\images\\ProfilePhoto_"+self.User+".jpg"):
        ...
        if self.PageID == '0': pass # Essa não tem cabeçalho

        elif self.PageIDAux == '9':
        ...
        elif self.PageID == '2' or self.PageID == '3' or self.PageID == '4' or self.PageID == '7':
        ...
        elif self.PageID in ['3b']:
            self.Button_Return.pressed.connect(lambda: self.CreatePage('3'))
            self.GLayout.addWidget(self.Background_Header, 0, 0, 1, 20)
        ...
        elif self.PageID in ['8b']:
            self.Button_Return.pressed.connect(lambda: self.CreatePage('3b'))
            self.GLayout.addWidget(self.Background_Header, 0, 0, 1, 20)
    ...
    def OnButtonPressed(self, ButtonPressed):
        ...
        if ButtonPressed == "ShowHideValues":
            ...
            if self.PageID == '11': # ARCA
                ...
                try:
                    ...
                    if self.HMI_ARCA.CarteiraGraph_id == 'pie': self.GraphWidget_Carteira = self.HMI_ARCA.CreateGraph_Page11_Carteira_pie()
                    else: self.GraphWidget_Carteira = self.HMI_ARCA.CreateGraph_Page11_Carteira_bar()
                ...
                try:
                    self.HMI_ARCA.Table_ContasCorrente.deleteLater()
                    self.HMI_ARCA.Table_ContasCorrente = QTableWidget()
                    self.HMI_ARCA.Table_ContasCorrente.setShowGrid(False)
                    self.HMI_ARCA.Table_ContasCorrente.setFont(self.font10)
                    self.HMI_ARCA.Table_ContasCorrente.setStyleSheet('''QTableWidget {background-color: rgb(0, 0, 0);
                                                                                      color: white;
                                                                                      border: 1px solid rgba(0, 0, 0, 0);}
                    ...
                    self.HMI_ARCA.Table_ContasCorrente.verticalHeader().hide()
                    self.HMI_ARCA.Table_ContasCorrente.horizontalHeader().hide()
                    self.F_GLayout[self.GCount-1].addWidget(self.HMI_ARCA.Table_ContasCorrente, 3, 4, 2, 2, Qt.AlignHCenter | Qt.AlignTop)
                    self.HMI_ARCA.CreateTable_11_2() # Table_ContasCorrente (Criada com uma thread)
                except: pass
                ...
                if self.ShowValues:
                    ...
                    self.TextBox_Msg6.setEchoMode(QLineEdit.Normal)
                else:
                    ...
                    self.TextBox_Msg6.setEchoMode(QLineEdit.Password)
            ...
            elif self.PageID == 'Bancos': # BancosPage
                if self.ShowValues:
                    self.TextBox_Msg2.setEchoMode(QLineEdit.Normal)
                else:
                    self.TextBox_Msg2.setEchoMode(QLineEdit.Password)
                ...
                if self.HMI_ARCA.MainGraphIsShowing == "Visão Geral":
                    ...
                    self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page50_MainGraph_TotalDeUserCoinPorCorretora()
                    self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 2, 1, 5, 1, Qt.AlignCenter)
                elif self.HMI_ARCA.MainGraphIsShowing == "Desempenho do ativo":
                    self.GraphWidget_MainGraph = self.HMI_ARCA.CreateGraph_Page50_MainGraph_DesempenhoDoAtivo()
                    self.F_GLayout[self.GCount-1].addWidget(self.GraphWidget_MainGraph, 1, 1, 6, 1, Qt.AlignCenter)
        ...
        #%% ButtonPressed == 'RemoveUnusedAtivosFromCotacoes'
        elif ButtonPressed == 'RemoveUnusedAtivosFromCotacoes':
            self.DBManager.RemoveUnusedAtivosFromCotacoes()
            self.CreatePage("45")
            ...
            if not self.filedialogIsOpen:
                ...
                if len(Excel_file_path)>0:
                    try:
                        ...
                        # Verifica se tem as folhas base
                        TableNames = workbook.sheetnames
                        corretoras = []
                        if not ("Bolsa" in TableNames and
                                "Cotações" in TableNames and
                                "Corretoras" in TableNames and
                                "Bancos" in TableNames): raise ValueError('''Faltam dados básicos.\nVerifique se consta as seguintes folhas no arquivo excel:\n
                                                                             01 - Bolsa\n
                                                                             02 - Cotações\n
                                                                             03 - Corretoras\n
                                                                             04 - Bancos''')
                        for sheet in TableNames: # Get corretoras
                            if "¨Bolsa¨DepSaq" in sheet:
                                corretora = sheet.replace("¨Bolsa¨DepSaq","")
                                corretora = corretora.replace("¨","")
                                corretoras.append(corretora+"¨Bolsa")
                            elif "OPs¨" in sheet and "¨Cripto¨Bruto" in sheet:
                                corretora = sheet.replace("¨Cripto¨Bruto","")
                                corretora = corretora.replace("OPs¨","")
                                corretoras.append(corretora+"¨Cripto")

                        # Verificar se tem a mesma quantidade de corretoras encontradas nas folhas também na folha de corretoras
                        CorretorasFromCorretoras = list(map(lambda cell: cell.value, workbook["Corretoras"]["A"]))[1:]
                        for corretora in corretoras:
                            if not corretora in CorretorasFromCorretoras: raise ValueError("Faltam dados.\n"+corretora+" possui folha Bruto, mas não consta na folha Corretoras.")
                        for corretora in CorretorasFromCorretoras:
                            if not corretora in corretoras: raise ValueError("Faltam dados.\n"+corretora+" consta na folha Corretoras, mas não possui a folha Bruto.")
                        if (any(CorretorasFromCorretoras.count(element) > 1 for element in CorretorasFromCorretoras) or
                            any(corretoras.count(element) > 1 for element in corretoras)):
                            raise ValueError("Há uma corretora duplicada.")

                        CorretorasFromCorretorasA = list(map(lambda cell: cell.value, workbook["Corretoras"]["A"]))[1:]
                        CorretorasFromCorretorasB = list(map(lambda cell: cell.value, workbook["Corretoras"]["B"]))[1:]
                        CorretorasFromCorretorasC = list(map(lambda cell: cell.value, workbook["Corretoras"]["C"]))[1:]
                        CorretorasFromCorretoras = []
                        for idx in range(len(CorretorasFromCorretorasA)):
                            if CorretorasFromCorretorasA[idx] != None or CorretorasFromCorretorasB[idx] != None or CorretorasFromCorretorasC[idx] != None:
                                CorretorasFromCorretoras.append((CorretorasFromCorretorasA[idx], CorretorasFromCorretorasB[idx], CorretorasFromCorretorasC[idx]))

                        UserName = self.TextBox_NomeDeUsuario.text()
                        UserPswd = self.TextBox_Senha1.text()
                        UserEmail = self.TextBox_Email.text().lower()
                        UserCoin = self.ComboBox_Currency.currentText().upper()
                        UserPath = self.TextBox_Path.text()
                        currdir = os.getcwd() # Get current directory
                        if UserPath == '': UserPath = currdir

                        # Import basis

                        # Create DB
                        try:
                            self.DBManager.User = UserName; self.DBManager.Senha = UserPswd; self.DBManager.Email = UserEmail; self.DBManager.UserPath = UserPath

                            # Create Usuários.db IF NOT EXISTS
                            self.DBManager.CriarDB(currdir+'\\Users.db')
                            # Connect to Users.db
                            self.DBManager.ConnectToDB(currdir+'\\Users.db')
                            # Create Table Usuários IF NOT EXISTS
                            self.DBManager.CreateTableInCurrentDB('Usuários', [('Usuário', 'TEXT'), ('Email', 'TEXT'), ('Path', 'TEXT')])
                            # Adicionar Usuário e Email
                            self.DBManager.AddRowInCurrentDB('Usuários', [('Usuário',UserName), ('Email',UserEmail), ('Path', UserPath)])
                            # Disconnect from Users.db
                            self.DBManager.Disconnect()
                            ...
                            # Criar user.db
                            self.DBManager.CriarDB(UserPath+"\\"+UserName+'.db')
                            # Connect to User.db
                            self.DBManager.ConnectToDB(UserPath+"\\"+UserName+'.db')
                            # Create Table Perfil
                            self.DBManager.CreateTableInCurrentDB('Perfil', [('Usuário', 'TEXT'), ('Senha','TEXT'), ('Email', 'TEXT'), ('TaxaB3Per','REAL'), ('MoedaCorrente','TEXT'), ('GastoMensal', 'REAL'), ('MesesDeReserva', 'REAL')])
                            # Adicionar Usuário, Senha, Email, TaxaB3Per = 0.03%
                            self.DBManager.AddRowInCurrentDB('Perfil', [('Usuário',UserName), ('Senha',UserPswd), ('Email',UserEmail), ('TaxaB3Per',0.03), ('MoedaCorrente',UserCoin), ('GastoMensal', 0), ('MesesDeReserva', 6)])
                            # Create Table Perfil_TiposDeAtivos
                            self.DBManager.CreateTableInCurrentDB('TiposDeAtivo', [('TipoDeAtivo', 'TEXT')])
                            # Adicionar os Tipos básicos
                            if "TiposDeAtivo" in TableNames: TiposDeAtivo = list(map(lambda cell: cell.value, workbook["TiposDeAtivo"]["A"]))[1:]
                            else: TiposDeAtivos = self.DBManager.TiposDeAtivosBasicos
                            for TipoDeAtivo in TiposDeAtivo:
                                self.DBManager.AddRowInCurrentDB('TiposDeAtivo', [('TipoDeAtivo', TipoDeAtivo)])
                            # Create Table Perfil_SubtipoDeAtivos
                            self.DBManager.CreateTableInCurrentDB('SubtiposDeAtivo', [('TipoDeAtivo', 'TEXT'),('SubtipoDeAtivo', 'TEXT')])
                            # Adicionar os Subtipos básicos
                            if "SubtiposDeAtivo" in TableNames:
                                SubtiposDeAtivoA = list(map(lambda cell: cell.value, workbook["SubtiposDeAtivo"]["A"]))[1:]
                                SubtiposDeAtivoB = list(map(lambda cell: cell.value, workbook["SubtiposDeAtivo"]["B"]))[1:]
                                SubtiposDeAtivo = []
                                for idx in range(len(SubtiposDeAtivoA)):
                                    if SubtiposDeAtivoA[idx] != None or SubtiposDeAtivoB[idx] != None:
                                        SubtiposDeAtivo.append((SubtiposDeAtivoA[idx], SubtiposDeAtivoB[idx]))
                            else: SubtiposDeAtivo = self.DBManager.SubtiposDeAtivosBasicos
                            for item in SubtiposDeAtivo:
                                TipoDeAtivo = item[0]
                                SubtipoDeAtivo = item[1]
                                self.DBManager.AddRowInCurrentDB('SubtiposDeAtivo', [('TiposDeAtivo', TipoDeAtivo),('SubtipoDeAtivo', SubtipoDeAtivo)])
                            # Create Table Setores
                            self.DBManager.CreateTableInCurrentDB('Setores', [('Setor', 'TEXT')])
                            # Adicionar os Setores básicos
                            if "Setores" in TableNames: Setores = list(map(lambda cell: cell.value, workbook["Setores"]["A"]))[1:]
                            else: Setores = self.DBManager.SetoresBasicos
                            for Setor in Setores:
                                self.DBManager.AddRowInCurrentDB('Setores', [('Setor', Setor)])
                            # Create Table Bolsa
                            self.DBManager.CreateTableInCurrentDB('Bolsa', [('Ativo', 'TEXT'),('TipoDeAtivo', 'TEXT'),('SubtipoDeAtivo', 'TEXT'),('Setor', 'TEXT')])
                            BolsaA = list(map(lambda cell: cell.value, workbook["Bolsa"]["A"]))[1:]
                            BolsaB = list(map(lambda cell: cell.value, workbook["Bolsa"]["B"]))[1:]
                            BolsaC = list(map(lambda cell: cell.value, workbook["Bolsa"]["C"]))[1:]
                            BolsaD = list(map(lambda cell: cell.value, workbook["Bolsa"]["D"]))[1:]
                            Bolsa = []
                            for idx in range(len(BolsaA)):
                                if BolsaA[idx] != None or BolsaB[idx] != None or BolsaC[idx] != None or BolsaD[idx] != None:
                                    Bolsa.append((BolsaA[idx], BolsaB[idx], BolsaC[idx], BolsaD[idx]))
                            # Create Table Cotações
                            self.DBManager.CreateTableInCurrentDB('Cotações', [('Modo', 'TEXT'),('DataDeAtualização', 'DATETIME'),('Ativo', 'TEXT'),('Cotação', 'REAL')])
                            if not UserCoin == "USD": self.DBManager.AddRowInCurrentDB('Cotações', [('Modo', 'Auto'),('DataDeAtualização', datetime.strptime(datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "%d/%m/%Y %H:%M:%S")),('Ativo', 'USD'),('Cotação', 0.)])
                            CotacoesA = list(map(lambda cell: cell.value, workbook["Cotações"]["A"]))[1:]
                            CotacoesB = list(map(lambda cell: cell.value, workbook["Cotações"]["B"]))[1:]
                            CotacoesC = list(map(lambda cell: cell.value, workbook["Cotações"]["C"]))[1:]
                            CotacoesD = list(map(lambda cell: cell.value, workbook["Cotações"]["D"]))[1:]
                            Cotacoes = []
                            for idx in range(len(CotacoesA)):
                                if CotacoesA[idx] != None or CotacoesB[idx] != None or CotacoesC[idx] != None or CotacoesD[idx] != None:
                                    Cotacoes.append((CotacoesA[idx], CotacoesB[idx], CotacoesC[idx], CotacoesD[idx]))
                            # Create Table Tributação
                            self.DBManager.CreateTableInCurrentDB('TributaçãoDTBolsa', [('MesAno', 'DATETIME'),('Resultado', 'REAL'),('PrejuizoAcumulado', 'REAL'),('ResultadoFinal', 'REAL'),('Alíquota', 'REAL'),('LucroMínimoTaxável', 'REAL'),('ImpostoDevido', 'REAL'),('ResultadoLíquido', 'REAL')])
                            self.DBManager.CreateTableInCurrentDB('TributaçãoSTBolsa', [('MesAno', 'DATETIME'),('Resultado', 'REAL'),('PrejuizoAcumulado', 'REAL'),('ResultadoFinal', 'REAL'),('Alíquota', 'REAL'),('LucroMínimoTaxável', 'REAL'),('ImpostoDevido', 'REAL'),('ResultadoLíquido', 'REAL')])
                            self.DBManager.CreateTableInCurrentDB('TributaçãoDTCripto', [('MesAno', 'DATETIME'),('Resultado', 'REAL'),('PrejuizoAcumulado', 'REAL'),('ResultadoFinal', 'REAL'),('Alíquota', 'REAL'),('LucroMínimoTaxável', 'REAL'),('ImpostoDevido', 'REAL'),('ResultadoLíquido', 'REAL')])
                            self.DBManager.CreateTableInCurrentDB('TributaçãoSTCripto', [('MesAno', 'DATETIME'),('Resultado', 'REAL'),('PrejuizoAcumulado', 'REAL'),('ResultadoFinal', 'REAL'),('Alíquota', 'REAL'),('LucroMínimoTaxável', 'REAL'),('ImpostoDevido', 'REAL'),('ResultadoLíquido', 'REAL')])
                            # Create Table Corretoras
                            self.DBManager.CreateTableInCurrentDB('Corretoras', [('Corretora', 'TEXT'),('MoedaCorrente', 'TEXT'),('ContaCorrente', 'REAL')])
                            CorretorasA = list(map(lambda cell: cell.value, workbook["Corretoras"]["A"]))[1:]
                            CorretorasB = list(map(lambda cell: cell.value, workbook["Corretoras"]["B"]))[1:]
                            CorretorasC = list(map(lambda cell: cell.value, workbook["Corretoras"]["C"]))[1:]
                            Corretoras = []
                            for idx in range(len(CorretorasA)):
                                if CorretorasA[idx] != None or CorretorasB[idx] != None or CorretorasC[idx] != None:
                                    Corretoras.append((CorretorasA[idx], CorretorasB[idx], CorretorasC[idx]))
                            # Create Table Bancos
                            self.DBManager.CreateTableInCurrentDB('Bancos', [('Banco', 'TEXT'),('MoedaCorrente', 'TEXT'),('ContaCorrente', 'REAL')])
                            BancosA = list(map(lambda cell: cell.value, workbook["Bancos"]["A"]))[1:]
                            BancosB = list(map(lambda cell: cell.value, workbook["Bancos"]["B"]))[1:]
                            BancosC = list(map(lambda cell: cell.value, workbook["Bancos"]["C"]))[1:]
                            Bancos = []
                            for idx in range(len(BancosA)):
                                if BancosA[idx] != None or BancosB[idx] != None or BancosC[idx] != None:
                                    Bancos.append((BancosA[idx], BancosB[idx], BancosC[idx]))
                            for banco in Bancos:
                                self.DBManager.AddRowInCurrentDB('Bancos', [('Banco', banco[0]), ('MoedaCorrente', banco[1]), ('ContaCorrente', banco[2])])
                            # Create Table MoedasFiat
                            self.DBManager.CreateTableInCurrentDB('MoedasFiat', [('MoedaFiat', 'TEXT')])
                            if "MoedasFiat" in TableNames: MoedasFiat = list(map(lambda cell: cell.value, workbook["MoedasFiat"]["A"]))[1:]
                            else: MoedasFiat = self.DBManager.MoedasFiatBasicas
                            for Moeda in MoedasFiat:
                                self.DBManager.AddRowInCurrentDB('MoedasFiat', [('MoedaFiat', Moeda)])
                        except:
                            print("Problema na criação da DB.")
                            raise ValueError("Houve algum problema na criação da DB.\nVerifique os dados na tabela a ser importada\ne tente novamente.")

                        if "Tickers" in TableNames:
                            Tickers = list(map(lambda cell: cell.value, workbook["Tickers"]["A"]))[1:]
                            self.DBManager.SetAllTickers(Tickers)
                        else:
                            print("Getting all tickers online")
                            self.YahooFinance.GetAllTickers()

                        for corretora, MoedaCorrente, ContaCorrente in CorretorasFromCorretoras:
                            if "¨Bolsa" in corretora:
                                BolsaOuCripto = "Bolsa"
                                Corretora = corretora.replace("¨Bolsa","")
                                self.DBManager.AddRowInCurrentDB('Corretoras', [('Corretora', Corretora+"¨"+BolsaOuCripto),('MoedaCorrente', MoedaCorrente),('ContaCorrente', ContaCorrente)])
                                # Create Table DepósitosESaques
                                self.DBManager.CreateTableInCurrentDB("¨"+Corretora+"¨"+BolsaOuCripto+"¨DepSaq", [('Data', 'DATETIME'),('Valor', 'REAL')])
                                # Importar ¨DepSaq
                                DepSaqA = list(map(lambda cell: cell.value, workbook["¨"+Corretora+"¨"+BolsaOuCripto+"¨DepSaq"]["A"]))[1:]
                                DepSaqB = list(map(lambda cell: cell.value, workbook["¨"+Corretora+"¨"+BolsaOuCripto+"¨DepSaq"]["B"]))[1:]
                                DepSaq = []
                                for idx in range(len(DepSaqA)):
                                    if DepSaqA[idx] != None or DepSaqB[idx] != None:
                                        DepSaq.append((DepSaqA[idx], DepSaqB[idx]))
                                for info in DepSaq:
                                    self.DBManager.AddRowInCurrentDB("¨"+Corretora+"¨"+BolsaOuCripto+"¨DepSaq", [('Data', info[0]),('Valor', info[1])])

                            elif "¨Cripto" in corretora:
                                BolsaOuCripto = "Cripto"
                                ...
                                self.DBManager.CreateTableInCurrentDB(TableNameRefinado, [('Observação', 'TEXT'),
                                                                                          ('Resultado', 'REAL'),
                                                                                          ('Data', 'DATETIME'),
                                                                                          ('Tipo', 'TEXT'),
                                                                                          ('ParEsquerdo', 'TEXT'),
                                ...
                                self.DBManager.CreateTableInCurrentDB(TableNameBruto, [('Observação', 'TEXT'),
                                                                                       ('Data', 'DATETIME'),
                                                                                       ('Tipo', 'TEXT'),
                                                                                       ('ParEsquerdo', 'TEXT'),
                                                                                       ('ParDireito', 'TEXT'),
                        ...
                        AtivosInseridos = []
                        for sheet in TableNames:
                            if "OPs¨" in sheet and "¨Bolsa¨Bruto" in sheet:
                                try:
                                    ativo = sheet.replace("OPs¨","").replace("¨Bolsa¨Bruto","")
                                    ativo = ativo[0:ativo.find("¨")]
                                    corretora = sheet.replace("OPs¨","").replace("¨Bolsa¨Bruto","").replace(ativo+"¨","")
                                    ModoManualAtivo = [item[0] for item in Cotacoes if item[2] == ativo][0]
                                    if not ModoManualAtivo == 'Manual': ativo = ativo.upper()
                                    if ativo not in AtivosInseridos:
                                        self.DBManager.AddRowInCurrentDB('Bolsa', [('Ativo', ativo.replace(" ","_")),
                                                                                   ('TipoDeAtivo', [item[1] for item in Bolsa if item[0] == ativo][0]),
                                                                                   ('SubtipoDeAtivo', [item[2] for item in Bolsa if item[0] == ativo][0]),
                                                                                   ('Setor', [item[3] for item in Bolsa if item[0] == ativo][0])])
                                        self.DBManager.AddRowInCurrentDB('Cotações', [('Modo', ModoManualAtivo),
                                                                                      ('DataDeAtualização', [item[1] for item in Cotacoes if item[2] == ativo][0]),
                                                                                      ('Ativo', ativo),
                                                                                      ('Cotação', [item[3] for item in Cotacoes if item[2] == ativo][0])])
                                        AtivosInseridos.append(ativo)
                                    TableName = str('OPs¨'+ativo.replace(" ","_")+
                                                    '¨'+corretora.replace(" ","_")+
                                                    '¨Bolsa')
                                    TableNameBruto = TableName+'¨Bruto'; TableNameDT = TableName+'¨DT'; TableNameST = TableName+'¨ST'
                                    self.DBManager.CreateTableInCurrentDB(TableNameDT, [('Data', 'DATETIME'),
                                                                                        ('Tipo', 'TEXT'),
                                                                                        ('Qqt', 'INTEGER'),
                                                                                        ('Preço', 'REAL'),
                                                                                        ('Corretagem', 'REAL'),
                                                                                        ...
                                                                                        ('CustoMédio', 'REAL'),
                                                                                        ('Resultado', 'REAL')])
                                    self.DBManager.CreateTableInCurrentDB(TableNameST, [('Data', 'DATETIME'),
                                                                                        ('Tipo', 'TEXT'),
                                                                                        ('Qqt', 'INTEGER'),
                                                                                        ('Preço', 'REAL'),
                                                                                        ('Corretagem', 'REAL'),
                                                                                        ...
                                                                                        ('CustoMédio', 'REAL'),
                                                                                        ('Resultado', 'REAL')])
                                    self.DBManager.CreateTableInCurrentDB(TableNameBruto, [('Data', 'DATETIME'),
                                                                                           ('Tipo', 'TEXT'),
                                                                                           ('Qqt', 'INTEGER'),
                                                                                           ('Preço', 'REAL'),
                                                                                           ('Corretagem', 'REAL'),
                                                                                           ...
                                                                                           ('CustoMédio', 'REAL'),
                                                                                           ('Resultado', 'REAL')])
                                    DadosA = list(map(lambda cell: cell.value, workbook[TableNameBruto]["A"]))[1:]
                                    DadosB = list(map(lambda cell: cell.value, workbook[TableNameBruto]["B"]))[1:]
                                    DadosC = list(map(lambda cell: cell.value, workbook[TableNameBruto]["C"]))[1:]
                                    DadosD = list(map(lambda cell: cell.value, workbook[TableNameBruto]["D"]))[1:]
                                    DadosE = list(map(lambda cell: cell.value, workbook[TableNameBruto]["E"]))[1:]
                                    DadosF = list(map(lambda cell: cell.value, workbook[TableNameBruto]["F"]))[1:]
                                    DadosG = list(map(lambda cell: cell.value, workbook[TableNameBruto]["G"]))[1:]
                                    DadosH = list(map(lambda cell: cell.value, workbook[TableNameBruto]["H"]))[1:]
                                    DadosI = list(map(lambda cell: cell.value, workbook[TableNameBruto]["I"]))[1:]
                                    DadosJ = list(map(lambda cell: cell.value, workbook[TableNameBruto]["J"]))[1:]
                                    DadosK = list(map(lambda cell: cell.value, workbook[TableNameBruto]["K"]))[1:]
                                    DadosL = list(map(lambda cell: cell.value, workbook[TableNameBruto]["L"]))[1:]
                                    DadosM = list(map(lambda cell: cell.value, workbook[TableNameBruto]["M"]))[1:]
                                    Dados = []
                                    for idx in range(len(DadosA)):
                                        if DadosA[idx] != None:
                                            Dados.append((DadosA[idx], DadosB[idx], DadosC[idx], DadosD[idx], DadosE[idx], DadosF[idx], DadosG[idx], DadosH[idx], DadosI[idx],DadosJ[idx], DadosK[idx], DadosL[idx], DadosM[idx]))
                                    for row in Dados:
                                        Obs = ''
                                        if row[7] != None: Obs = row[7]
                                        self.DBManager.AddRowInCurrentDB(TableNameBruto, [('Data', row[0]),
                                                                                          ('Tipo', row[1]),
                                                                                          ('Qqt', row[2]),
                                                                                          ('Preço', row[3]),
                                                                                          ('Corretagem', row[4]),
                                                                                          ...
                                                                                          ('CustoMédio', row[11]),
                                                                                          ('Resultado', row[12])])
                                        if not 'Fundos Imobiliários' == [item[2] for item in Bolsa if item[0] == ativo][0]:
                                            self.DBManager.AddRowInCurrentDB(TableNameST, [('Data', row[0]),
                                                                                           ('Tipo', row[1]),
                                                                                           ('Qqt', row[2]),
                                                                                           ('Preço', row[3]),
                                                                                           ('Corretagem', row[4]),
                                    ...
                                    if not 'Fundos Imobiliários' == [item[2] for item in Bolsa if item[0] == ativo][0]:
                                        self.DBManager.SepararDTeST(TableName)
                                        self.DBManager.CalcularCelsAutomatizadas(TableNameBruto)
                                        self.DBManager.CalcularCelsAutomatizadas(TableNameDT)
                                        self.DBManager.CalcularCelsAutomatizadas(TableNameST)
                                    else:
                                        self.DBManager.CopiarBrutoPraDT(TableName)
                                        self.DBManager.CalcularCelsAutomatizadas(TableNameBruto)
                                        self.DBManager.CalcularCelsAutomatizadas(TableNameDT)
                                except: raise ValueError("Verifique se a tabela de cotações contém\ntodos os ativos a serem processados.")

                            elif "OPs¨" in sheet and "¨Cripto¨Bruto" in sheet:
                                def ThereIsAnotherOPWithTheSameDate(TableName, data):
                                    Table_Inicial, cursor = self.DBManager.GetDataDB("SELECT rowid, * FROM "+TableName+"¨Bruto")
                                ...
                                corretora = sheet.replace("OPs¨","").replace("¨Cripto¨Bruto","")
                                TableName = str('OPs'+
                                                '¨'+corretora.replace(" ","_")+
                                                '¨Cripto')
                                TableNameBruto = TableName+'¨Bruto'
                                ...
                                for idx in range(len(DadosA)):
                                    if DadosC[idx] != None:
                                        ...
                                        ParDireito = ''
                                        Conversao = 1
                                        if DadosA[idx] != None: Obs = DadosA[idx]
                                        if DadosE[idx] != None: ParDireito = DadosE[idx]
                                        if DadosJ[idx] != None: Conversao = DadosJ[idx]
                                        Dados.append((Obs, DadosB[idx], DadosC[idx], DadosD[idx], ParDireito, DadosF[idx], DadosG[idx], DadosH[idx], DadosI[idx], Conversao))

                                for row in Dados:
                                    if not isinstance(row[1], datetime): data = datetime.strptime(row[1], '%Y-%m-%d %H:%M:%S')
                                    else: data = row[1]
                                    if ThereIsAnotherOPWithTheSameDate(TableName, data): raise ValueError("Há uma operação na mesma data.\nVerifique em "+TableNameBruto+" no momento "+data.strftime("%d/%m/%Y %H:%M:%S")+"\ne tente novamente.")
                                    if data > datetime.now(): raise ValueError("Há uma operação ocorrendo no futuro.\nArrume isso em "+TableNameBruto+" no momento "+data.strftime("%d/%m/%Y %H:%M:%S")+"\ne tente novamente.")

                                    Taxa = 0; MoedaDaTaxa = UserCoin; Conversao = 1; ParDireito = ''

                                    if row[7]>0: Taxa = round(row[7],8)
                                    if len(row[8])>0: MoedaDaTaxa = row[8]
                                    if row[9]>0: Conversao = round(row[9],8)
                                    if len(row[4])>0: ParDireito = row[4]

                                    Obs = ''
                                    if row[0] != None: Obs = row[0]
                                    self.DBManager.AddRowInCurrentDB(TableNameBruto, [('Observação', Obs),
                                                                                      ('Data', data),
                                                                                      ('Tipo', row[2]),
                                                                                      ('ParEsquerdo', row[3]),
                                                                                      ('ParDireito', ParDireito),
                                                                                      ('Preço', round(row[5],8)),
                                                                                      ('Qqt', round(row[6],8)),
                                                                                      ('Taxa', Taxa),
                                ...
                                self.DBManager.CalcularCelsAutomatizadas_CriptoRefinado(TableName)

                                def GetRowId(corretora):
                                    retorno, cursor = self.DBManager.GetDataDB("SELECT rowid, * FROM Corretoras")
                                ...
                                Valor = self.DBManager.GetEstoque(ativo = UserCoin, corretora = corretora, BolsaOuCripto = "Cripto")
                                self.DBManager.ModifyDB("UPDATE Corretoras SET ContaCorrente = (?) Where rowid = "+GetRowId(corretora), [(Valor)])

                                # Verificar se os dois ativos estão na Cotações (DB). Se não estiverem, coloque e atualize a cotação se for encontrado online.
                                def AtivoExisteNaTabelaDeCotacoes(ativo):
                                    Table, cursor = self.DBManager.GetDataDB("SELECT * FROM Cotações")
                                ...
                                TableBruto, cursor = self.DBManager.GetDataDB("SELECT * FROM "+TableNameBruto)
                                ...
                                ListaDeMoedas = list(map(lambda coin: coin.upper(), ListaDeMoedas))
                                ListaDeMoedas = list(dict.fromkeys(ListaDeMoedas))
                                try:ListaDeMoedas.remove('')
                                except:pass
                                try:ListaDeMoedas.remove(None)
                                except:pass
                                qqtMoedasNovas = 0

                                for coin in ListaDeMoedas:
                                    if (not coin == UserCoin and
                                        not AtivoExisteNaTabelaDeCotacoes(coin) and
                                        not AtivoExisteNaFolhaDeCotacoes(coin, Cotacoes)):
                                        qqtMoedasNovas += 1
                                if qqtMoedasNovas == 1: MessageBox_Msg1 = QMessageBox.about(self,'Aviso',"Identificamos uma nova moeda na base de dados proveniente da\n"+TableNameBruto+".\nA operação seguinte pode levar alguns minutos,\npois irá buscar online a cotação atual da moeda.\nNÃO FECHE O PROGRAMA DURANTE O PROCESSO SEGUINTE!")
                                elif qqtMoedasNovas > 1: MessageBox_Msg1 = QMessageBox.about(self,'Aviso',"Identificamos novas moedas na base de dados ("+str(qqtMoedasNovas)+") proveniente da\n"+TableNameBruto+".\nA operação seguinte pode levar alguns minutos,\npois irá buscar online a cotação atual de cada moeda.\nNÃO FECHE O PROGRAMA DURANTE O PROCESSO SEGUINTE!")

                                thread_list = []
                                for coin in ListaDeMoedas:
                                    thread_list.append(BP(ThreadName='Thread_GetCotacaoCripto_FromExcelImportation',
                                                          HMI=self,
                                                          ativo=coin,
                                                          currency=UserCoin,
                                                          Cotacoes=Cotacoes))
                        ...
                        # self.DBManager.CalcularTributacao() # Via thread
                        self.HMI_Tributacao.CalcularTributacao_SemThread() # Via sequencial, mas tem que comentar as linhas que tem self.HMI.DBManager.LW... em HMI_Tributação.py # Debug
                        self.DBManager.UpdatePatrimonioRendimento()

                        MessageBox_Msg1 = QMessageBox.about(self,'Informação','Importação bem sucedida.\nBem-vindo de volta '+UserName+'\nAcesse sua conta entrando pela página inicial.')
                    except Exception as msg:
                        if "Operação cancelada." in repr(msg):
                            self.DBManager.Disconnect()
                            MessageBox_Msg1 = QMessageBox.about(self,'Informação','Importação cancelada.')
                        else:
                            try:
                                self.DBManager.DeleteUserDB()
                            except:
                                self.DBManager.Disconnect()
                                print("Problema ao cancelar o processo...\nArquivo de usuário pode ter sido criado, mas deve estar corrompido.")
        ...
        if event.type() == QEvent.Resize:
            try:
                ...
                #%% Resizing HMI Page 41
                    ...
                    self.Button_BuscarConversao.setFixedWidth(int(self.frameGeometry().width()/7*0.9))
                    self.TextBox_Obs.setFixedWidth(int(self.frameGeometry().width()*4/7*0.9))
                    self.Button_LimparCampos.setFixedSize(int(self.frameGeometry().width()*2/10*0.9),int(self.frameGeometry().width()*1/10*0.9))
                    self.Button_RegistrarOperacao.setIconSize(QSize(int(self.frameGeometry().height()/10*0.9),int(self.frameGeometry().height()/10*0.9)))
                    self.Button_RegistrarOperacao.setFixedSize(int(self.frameGeometry().width()*2/10*0.9),int(self.frameGeometry().width()*1/10*0.9))
