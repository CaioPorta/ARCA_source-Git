...
class YahooFinance(object):
    ...
    def GetAllTickers(self): # Thread
        ...
        try:self.HMI.DBManager.SetAllTickers(List_AllTickers)
        except:pass
    ...
    def GetCotacao(self, ativo, currency):
        ...
        try:
            ...
            try: # Try method 1
                ...
                data = get_data(ativo+".SA", start_date=start_date)
                ...
                try: # Try method 2
                    ...
                    try: # Try method 3
                        ...
                        data = get_data(ativo, start_date=start_date)
                        UltimaCotacao = round(float(data["close"][-1]),2)
                    except:
                        try: # Try method 4
    ...
    def GetCotacaoCripto(self, coin, currency, date=''):
        ...
        def GetCoinCotacaoEmUSD(coin):
            ...
            cotacao = 0.
            if coin in self.HMI.DBManager.GetMoedasFiat():
                try:
                    start_date = datetime.strptime((datetime.now()-timedelta(hours=2)).strftime("%Y/%m/%d %H:%M:%S"), "%Y/%m/%d %H:%M:%S")
                    data = get_data(coin+"USD=X", start_date=start_date)
                    cotacao = round(data["close"][-1],8)
                except:
                    try: # Try second method
                        data = web.get_data_yahoo(coin+"USD=X", start=start_date) # Não funciona bem, apresenta BUGs em momentos aleatórios, mas pode ser uma solução
                        cotacao = round(data["Close"][-1],8)
                    except:
                        print("Erro na recuperação da cotação do ativo "+coin+".\nFunção geradora do erro: GetCoinCotacaoEmUSD em YahooFinance.py")
            else:
                try: # Try first method
                    start_date = datetime.strptime((datetime.now()-timedelta(hours=2)).strftime("%Y/%m/%d %H:%M:%S"), "%Y/%m/%d %H:%M:%S")
                    data = get_data(coin+"-USD", start_date=start_date)
                    cotacao = round(data["close"][-1],8)
                except:
                    try: # Try second method
                        ...
                        print("Erro na recuperação da cotação do ativo "+coin+".\nFunção geradora do erro: GetCoinCotacaoEmUSD em YahooFinance.py")
            return round(cotacao, 8)

        def GetDolarCotacao_EmDataEspecifica(currency, date): # Usei requests pq funciona melhor pra cotação do dólar, dá menos problema
            url = "https://finance.yahoo.com"; timeout = 5
            try: request = requests.get(url, timeout=timeout) # Testa conexão com a internet
            except (requests.ConnectionError, requests.Timeout) as exception: return 0.
            Cotacao = 0.
            try:
                end_date = date
                end_date_dt = datetime.strptime(date, "%Y/%m/%d %H:%M:%S")
                start_date = (end_date_dt - timedelta(minutes=4)).strftime("%Y/%m/%d %H:%M:%S")
                dados = client.get_historical_klines("USDT"+currency, interval="1m", start_str=start_date,end_str=end_date)
                Cotacao = float(dados[-1][4]) # Fechamento
            except: print("Erro na recuperação da cotação do dólar.\nFunção geradora do erro: GetCotacaoCripto_EmDataEspecifica, GetDolarCotacao em YahooFinance.py")
            finally: return round(Cotacao, 3)

        def GetCoinCotacaoEmUSD_EmDataEspecifica(coin, date):
            url = "https://finance.yahoo.com"; timeout = 5
            try: request = requests.get(url, timeout=timeout) # Testa conexão com a internet
            except (requests.ConnectionError, requests.Timeout) as exception: return 0.
            cotacao = 0.
            end_date = datetime.strptime(date, "%Y/%m/%d %H:%M:%S")
            start_date = datetime.strptime((end_date-timedelta(minutes=4)).strftime("%Y/%m/%d %H:%M:%S"), "%Y/%m/%d %H:%M:%S")
            if coin in self.HMI.DBManager.GetMoedasFiat():
                try:
                    data = get_data(coin+"USD=X", start_date=start_date, end_date=end_date, interval = "1m")
                    cotacao = round(data["close"][-1],8)
                except:
                    try: # Try second method
                        data = web.get_data_yahoo(coin+"USD=X", start=start_date, end=end_date, interval = "1m") # Não funciona bem, apresenta BUGs em momentos aleatórios, mas pode ser uma solução
                        cotacao = round(data["Close"][-1],8)
                    except:
                        print("Erro na recuperação da cotação do ativo "+coin+".\nFunção geradora do erro: GetCoinCotacaoEmUSD em YahooFinance.py")
            else:
                try: # Try first method
                    data = get_data(coin+"-USD", start_date=start_date, end_date=end_date, interval = "1m")
                    cotacao = round(data["close"][-1],8)
                except:
                    try: # Try second method
                        data = web.get_data_yahoo(coin+"-USD", start=start_date, end=end_date, interval = "1m") # Não funciona bem, apresenta BUGs em momentos aleatórios, mas pode ser uma solução
                        cotacao = round(data["Close"][-1],8)
                    except:
                        print("Erro na recuperação da cotação do ativo "+coin+".\nFunção geradora do erro: GetCotacaoCripto_EmDataEspecifica, GetCoinCotacaoEmUSD em YahooFinance.py")
            return round(cotacao, 8)

        CotacaoCoinEmCurrency = 0.
        # print("\nBuscando cotação de ",coin) # Debug
        if coin == currency: CotacaoCoinEmCurrency = 1.
        else:
            if date == '':
                if currency in ["USD", "BUSD", "USDT", "USDC", "USDP", "TUSD"]: CotacaoUSD = 1. # O próprio dólar em dólar
                else: CotacaoUSD = GetDolarCotacao(currency) # Cotação do dólar na moeda corrente

                if coin in ["USD", "BUSD", "USDT", "USDC", "USDP", "TUSD"]: CotacaoCoinEmUSD = 1. # A cotação já está em dólar
                else: CotacaoCoinEmUSD = GetCoinCotacaoEmUSD(coin)
            else:
                if currency in ["USD", "BUSD", "USDT", "USDC", "USDP", "TUSD"]: CotacaoUSD = 1. # O próprio dólar em dólar
                else: CotacaoUSD = GetDolarCotacao_EmDataEspecifica(currency, date) # Cotação do dólar na moeda corrente

                if coin in ["USD", "BUSD", "USDT", "USDC", "USDP", "TUSD"]: CotacaoCoinEmUSD = 1. # A cotação já está em dólar
                else: CotacaoCoinEmUSD = GetCoinCotacaoEmUSD_EmDataEspecifica(coin, date)

            CotacaoCoinEmCurrency = round(CotacaoCoinEmUSD * CotacaoUSD, 8) # Converte para a moeda corrente

        # print('Cotacao '+coin+' em '+currency+': ', CotacaoCoinEmCurrency) # Debug
        return CotacaoCoinEmCurrency, CotacaoUSD
    ...
    def isShare(self, ShareToBeChecked):
        ...
        url = "https://finance.yahoo.com"; timeout = 5
        try: request = requests.get(url, timeout=timeout) # Testa conexão com a internet
        except (requests.ConnectionError, requests.Timeout) as exception: return IsShare, 0
